...
class Config:
    ...
    def _validate_config(self):
        ...
        if "gpt-4o-mini" in self.OPENAI_MODEL:
            max_allowed_tokens = 128000  # GPT-4o-mini has very high token limit
        elif "gpt-3.5" in self.OPENAI_MODEL:
            max_allowed_tokens = 4000  # GPT-3.5-turbo limit
        else:
            max_allowed_tokens = 128000  # Default to higher limit for newer models
            
        if self.MAX_TOKENS > max_allowed_tokens:
            raise ValueError(f"MAX_TOKENS cannot exceed {max_allowed_tokens} for {self.OPENAI_MODEL}")
